...
st.set_page_config(page_title="Ogi Social", layout="wide")
    ...
    if email and password:
        ...
        with col1:
            if st.button("Login", type="primary", use_container_width=True):
                ...
                response = requests.post("https://socialsimple-1.onrender.com/auth/jwt/login", data=login_data)
                ...
                if response.status_code == 200:
                    ...
                    # Get user info
                    user_response = requests.get("https://socialsimple-1.onrender.com/users/me", headers=get_headers())
        ...
        with col2:
            if st.button("Sign Up", type="secondary", use_container_width=True):
                ...
                response = requests.post("https://socialsimple-1.onrender.com/auth/register", json=signup_data)
...
def upload_page():
    ...
    if uploaded_file and st.button("Share", type="primary"):
        with st.spinner("Uploading..."):
            ...
            response = requests.post("https://socialsimple-1.onrender.com/upload", files=files, data=data, headers=get_headers())
...
def create_transformed_url(original_url, transformation_params, caption=None):
    if caption:
        ...
        text_overlay = f"l-text,ie-{encoded_caption},ly-N20,lx-20,fs-100,co-800080,bg-FFFF00A0,l-end"
...
def feed_page():
    ...
    response = requests.get("https://socialsimple-1.onrender.com/feed", headers=get_headers())
    if response.status_code == 200:
            ...
            with col2:
                if post.get('is_owner', False):
                    if st.button("🗑️", key=f"delete_{post['id']}", help="Delete post"):
                        # Delete the post
                        response = requests.delete(f"https://socialsimple-1.onrender.com/posts/{post['id']}", headers=get_headers())
                        if response.status_code == 200:
